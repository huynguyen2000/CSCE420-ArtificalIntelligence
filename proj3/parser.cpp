...
Expr::Expr(string s) { 
  // weird bug, something is adding a invis character at the end that messes up cout, this removes any trailing character that isn't alphanumeric
  sym = (isalnum(s.at(s.size() - 1)) ? s : s.substr(0, s.size() - 1)); 
  kind = ATOM; 
}
...
Expr::Expr(Expr* e)
{
  ...
  if (e->kind==ATOM){
    // weird bug, something is adding a invis character at the end that messes up cout, this removes any trailing character that isn't alphanumeric
    sym = (isalnum(e->sym.at(e->sym.size() - 1)) ? e->sym : e->sym.substr(0, e->sym.size() - 1)); 
  }
  else
  {
...
{
  if (a->kind==ATOM) {
    if (b->kind!=ATOM || a->sym.compare(b->sym) != 0) return false; }
  else // a->kind==LIST
  {
